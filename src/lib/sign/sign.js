...
const validateParams = (params) => {
  ...
  if (
    ...
    throw new Error(
      'Invalid uri format (must be https://example.com/login)'
    );
  }
  ...
  if (
    ...
    !(params.expiration_time instanceof Date)
  ) {
...
const processParams = (params) => {
  ...
  if (params.expires_in && !params.expiration_time) {
    params.expiration_time = new Date(
  ...
  if (params.chain_id) {
    params.chain_id = parseInt(params.chain_id);
  }
  ...
  if (!params.nonce) {
    params.nonce = parseInt((Math.random() * 99999999).toString());
  }
};
...
const buildMessage = (params) => {
  ...
  const param_labels = {
    ...
    'Wallet Type': params.wallet_type,
  };
